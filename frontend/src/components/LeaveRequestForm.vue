    ...
    <div class="modal-panel" style="max-width:480px;">
      ...
      <div class="modal-body">
        ...
        <select v-model="form.shiftId" class="form-input">
          <option value="">请选择要申请休假的班次</option>
        ...
        <label>休假原因</label>
        <textarea v-model="form.reason" class="form-input" rows="3" placeholder="请填写休假原因"></textarea>

        <p v-if="error" class="form-error">{{ error }}</p>
      </div>
      <div class="modal-footer">
        <button class="btn-ghost" @click="$emit('close')">取消</button>
        <button class="btn-primary btn-submit" :disabled="submitting || !form.shiftId" @click="submit">
          {{ submitting ? '提交中...' : '提交申请' }}
        </button>
      </div>
    </div>
...
.modal-panel { width: min(480px, 92vw); background: #fff; border-radius: 16px; box-shadow: 0 20px 60px rgba(66,44,160,0.25); overflow: hidden; }
...
.shift-preview { background: #f5f3ff; border-radius: 10px; padding: 12px; margin-top: 4px; }
...
.btn-submit:hover { box-shadow: 0 4px 12px rgba(16,185,129,0.3); }
