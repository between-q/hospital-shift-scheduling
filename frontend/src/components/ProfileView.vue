<template>
  ...
  <div class="profile-grid">
    ...
    <div class="card profile-panel">
      ...
      <div class="action-grid">
        <button class="secondary" @click="showRoleModal = true">权限管理</button>
      </div>
    </div>
  </div>
  ...
  <!-- 权限管理弹窗 -->
  <div v-if="showRoleModal" class="modal-mask" @click.self="showRoleModal = false">
    <div class="modal-panel" style="max-width: 420px;">
      ...
      <div class="modal-body">
        <label>选择用户</label>
        <select v-model="roleForm.userId" class="form-select">
          <option value="">请选择用户</option>
          <option v-for="u in adminUsers" :key="u.id" :value="u.id">
            {{ u.fullName || u.email }}
          </option>
        ...
        <div class="role-checkboxes">
          <label class="role-check">
            <input type="checkbox" :checked="roleForm.roles.includes('ADMIN')" @change="toggleRole('ADMIN')" />
            <span>管理员 (ADMIN)</span>
          </label>
          <label class="role-check">
            <input type="checkbox" :checked="roleForm.roles.includes('DOCTOR')" @change="toggleRole('DOCTOR')" />
            <span>医生 (DOCTOR)</span>
          </label>
          <label class="role-check">
            <input type="checkbox" :checked="roleForm.roles.includes('NURSE')" @change="toggleRole('NURSE')" />
            <span>护士 (NURSE)</span>
          </label>
        </div>
      ...
      <div class="modal-footer">
        ...
        <button class="btn-primary" :disabled="savingRoles" @click="submitRoles">
          {{ savingRoles ? '保存中...' : '保存' }}
        </button>
      </div>
    </div>
...
const emit = defineEmits(['navigate', 'reset-password', 'update-shift', 'update-roles']);
...
const roleForm = reactive({ userId: '', roles: [] });
...
const openRoleModal = (user) => {
  ...
  roleForm.roles = user.roles ? [...user.roles] : [];
  roleError.value = '';
  showRoleModal.value = true;
};

const toggleRole = (role) => {
  const idx = roleForm.roles.indexOf(role);
  if (idx >= 0) {
    roleForm.roles.splice(idx, 1);
  } else {
...
const submitRoles = async () => {
  if (!roleForm.userId || roleForm.roles.length === 0) {
  ...
  roleError.value = '';
  try {
    emit('update-roles', { userId: Number(roleForm.userId), roles: [...roleForm.roles] });
    showRoleModal.value = false;
  } catch (e) {
    roleError.value = e?.message || '更新失败';
  } finally {
...
.role-checkboxes { display: flex; flex-direction: column; gap: 8px; margin-top: 4px; }
.role-check { display: flex; align-items: center; gap: 8px; font-size: 14px; cursor: pointer; }
.role-check input { width: 16px; height: 16px; accent-color: #6366f1; }
</style>
