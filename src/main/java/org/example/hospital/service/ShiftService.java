...
public class ShiftService {
    ...
    private final ShiftConstraintService constraintService;

    public ShiftService(ShiftRepository shiftRepository,
                        DepartmentService departmentService,
                        UserAccountRepository userAccountRepository,
                        RealtimePublisher realtimePublisher,
                        ShiftConstraintService constraintService) {
    ...
    /**
     * 指派员工到班次（带完整约束检查）
     * 硬约束：角色匹配、班次重叠、每周最大工时
     * 中等约束：最短休息时间
     */
    @Transactional
    public ShiftResponse assignShift(Long shiftId, Long employeeId) {
        logger.info("指派班次：shiftId={}, employeeId={}", shiftId, employeeId);
        ...
        // 硬约束 1: 角色匹配
        ensureRoleMatches(employee, shift.getRequiredRole());

        // 硬约束 2: 班次重叠检查
        constraintService.checkShiftOverlap(employee, shift.getStartTime(), shift.getEndTime(), null);

        // 硬约束 3: 每周最大工时检查（45 小时）
        constraintService.checkWeeklyMaxHours(employee, shift.getStartTime(), shift.getEndTime());

        // 中等约束：最短休息时间检查（8 小时）
        constraintService.checkMinimumBreak(employee, shift.getStartTime());
        ...
        logger.info("班次指派成功：shiftId={}, employee={}", shiftId, employee.getFullName());
        ShiftResponse response = toResponse(saved);
        realtimePublisher.publishShiftEvent("SHIFT_ASSIGNED", response);
        return response;
    }
    ...
    public ShiftResponse updateAssignment(Long shiftId, UpdateShiftAssignmentRequest request) {
        ...
        if (request.getAssigneeUserId() != null) {
            ...
            // 硬约束 2: 班次重叠检查
            constraintService.checkShiftOverlap(assignee, shift.getStartTime(), shift.getEndTime(), shiftId);

            // 硬约束 3: 每周最大工时检查
            constraintService.checkWeeklyMaxHours(assignee, shift.getStartTime(), shift.getEndTime());

            // 中等约束：最短休息时间检查
            constraintService.checkMinimumBreak(assignee, shift.getStartTime());
    ...
    public ShiftResponse updateShiftDetails(Long shiftId, UpdateShiftRequest request) {
        ...
        if (request.getAssigneeUserId() != null) {
            ...
            // 硬约束 2: 班次重叠检查
            constraintService.checkShiftOverlap(assignee, request.getStartTime(), request.getEndTime(), shiftId);

            // 硬约束 3: 每周最大工时检查
            constraintService.checkWeeklyMaxHours(assignee, request.getStartTime(), request.getEndTime());

            // 中等约束：最短休息时间检查
            constraintService.checkMinimumBreak(assignee, request.getStartTime());
     ...
     */
    @Transactional(readOnly = true)
    public long getEmployeeWeeklyHours(Long employeeId, LocalDateTime referenceDate) {
        UserAccount employee = userAccountRepository.findById(employeeId)
                .orElseThrow(() -> new IllegalArgumentException("员工不存在"));
        return constraintService.getWeeklyHours(employee, referenceDate);
    }
     ...
     */
    @Transactional(readOnly = true)
    public boolean checkEmployeeWeeklyMinHours(Long employeeId, LocalDateTime referenceDate) {
        UserAccount employee = userAccountRepository.findById(employeeId)
                .orElseThrow(() -> new IllegalArgumentException("员工不存在"));
        return constraintService.checkWeeklyMinHours(employee, referenceDate);
    }
