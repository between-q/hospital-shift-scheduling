...
public class UserService {
    ...
    @Transactional
    public void updateRoles(Long userId, Set<RoleType> roleTypes) {
        logger.info("更新用户角色: {} -> {}", userId, roleTypes);
        UserAccount user = userAccountRepository.findById(userId)
                .orElseThrow(() -> {
                    logger.warn("用户不存在: {}", userId);
                    return new IllegalArgumentException("User not found");
        ...
        for (RoleType rt : roleTypes) {
            Role role = roleRepository.findByName(rt)
                    .orElseThrow(() -> new IllegalArgumentException("Role not found: " + rt));
            roles.add(role);
        }
        user.setRoles(roles);
        logger.info("用户角色更新成功: {}", userId);
    }
